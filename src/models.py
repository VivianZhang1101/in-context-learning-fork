...
class SingleLayerAttentionModel(nn.Module):
    def __init__(self, n_dims, n_positions, n_embd=64, n_layer=1, n_head=1):
        super(SingleLayerAttentionModel, self).__init__()
        self.name = f"single_layer_embd={n_embd}_layer={n_layer}_head={n_head}"
    ...
    def forward(self, xs, ys, inds=None):
        ...
        embeds += self.positional_encoding[:, :embeds.size(1), :]
        embeds = embeds.transpose(0, 1)  # Transpose for multihead attention
        output, _ = self._backbone(embeds, embeds, embeds)
        output = output.transpose(0, 1)  # Transpose back to original dimensions
        prediction = self._read_out(output)
        return prediction[:, ::2, 0][:, inds]  # Assuming prediction on xs
